...
def run(xz,deviceid,switch_ip,password,phmode,t,gj_mode,chvarUn,country,gj,qh,login_mode,privince,filtering_mode):
    rm = mode(deviceid,password,switch_ip,phmode,t,gj_mode,chvarUn,country,gj,qh,login_mode,privince,filtering_mode)
    ...
    if jg == None:
        ...
        if int(xcs)>= int(device_list.__len__()):
            for i in range(0,device_list.__len__()):
                threading.Thread(target=run, args=(xz,device_list[i], switch_ip, password, phmode, t, gj_mode, chvarUn, country, gj, qh, login_mode, privince, filtering_mode)).start()
        else:
...
def qd():
    ...
    win.title("火箭注册v0.9.6")  # 在这里修改窗口的标题
